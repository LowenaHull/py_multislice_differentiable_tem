...
# # Optimisation: Crystalline structure, MSE calculated using images
...
def create_working_dir():
    directory = "Run_1"
    parent_dir = '/home/hremadmin/Documents/Project/ML_files/Optimisation_Using_Images/'
...
def generate_experimental_data(GT_defocus, aperture, Cs, noise, working_dir):
    ...
    aberrations = establish_aberrations(Cs, GT_defocus)
...
def log_exp_conditions(learning_rate, loss_modulus, eV, Cs, aperture, distance_from_GT, n_iters, working_dir):
    ...
    file.write('Learning Rate: {lr}\nLoss Modulus: {lm}\nEnergy: {e} keV\nCs: {cs} mm\nAperture: {ap} mrad\n{x} away from GT\n{y} iterations'.format(lr=learning_rate, lm=loss_modulus, e=e_kV, cs=Cs, ap=aperture, x=distance_from_GT, y=n_iters))
    file.close()
...
def main(n_iters, learning_rate, loss_multiplier, GT_defocus, aperture, Cs, distance_from_GT, noise, working_dir):
    ...
    runtime, loss_data, ab_data = optimise(aberrations, output, initial_df, ref_data, n_iters, learning_rate, loss_multiplier, GT_defocus, aperture, Cs, df, working_dir)
...
def optimise(aberrations, output, initial_df, ref_data, n_iters, learning_rate, loss_multiplier, GT_defocus, aperture, Cs, df, working_dir):
    """Go through and refine the aberrations to minimise difference between initial guess (output) and experimental image"""

    #ref_data = ref_data.detach().cpu().numpy() 
    ref_data = ref_data.to(torch.float64)
    ...
    for iter in range(n_iters):
        ...
        sim_image = hrtem_image[0].to(torch.float64) #.detach().cpu().numpy()
...
# First is defocus values (cases A-D)
# Second is aperture
# Third is Cs
...
# For example, this will run 5 tests:
# 1. Standard conditions, 300 Å from the GT, 5000 iterations, 0.1 learning rate
# 2. Reduced Cs conditions, 150 Å from the GT, 3000 iterations, 1 learning rate
# 3. Increased Cs conditions, 500 Å from the GT, 1000 iterations, 10 learning rate
# 4. Standard conditions, 300 Å from the GT, 5000 iterations, 0.1 learning rate, 0.25 Gaussian noise
# 5. Standard conditions, 300 Å from the GT, 5000 iterations, 0.1 learning rate, 15 Gaussian noise

experimental_conditions = [
    [[-75.0, -425.0, -1475.0, -4975.0], 7.5, 1e7, 300.0, None, 5000, 1e-1],
    [[-75.0, -270.0, -860.0, -2825.0], 10.0, 0.25e7, 150.0, None, 3000, 1e0],
    [[-50.0, -500.0, -1900.0, -6575.0], 6.5, 2e7, 500.0, None, 1000, 1e1],
    [[-75.0, -425.0, -1475.0, -4975.0], 7.5, 1e7, 300.0, "G0.25", 5000, 1e-1],
    [[-75.0, -425.0, -1475.0, -4975.0], 7.5, 1e7, 300.0, "G15.0", 5000, 1e-1]
]
...
for i in range(len(experimental_conditions)):
    ...
    log_exp_conditions(learning_rate, loss_multiplier, eV, Cs/1e7, aperture, distance_from_GT, n_iterations, working_dir)
    
    for df in defocus_values:
        runtime, loss_data, ab_data = main(n_iterations, learning_rate, loss_multiplier, df, aperture, Cs, distance_from_GT, noise, working_dir)
    ...
    all_final_defocuses.append(final_defocuses)
